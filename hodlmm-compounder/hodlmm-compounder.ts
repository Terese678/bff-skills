...
function getSignedBinId(unsignedBinId: number): number {
...
async function loadWallet(): Promise<{
  ...
  if (!address) {
    throw new Error(
      "No wallet address found. Ensure AIBTC MCP wallet is installed and STACKS_ADDRESS env is set."
    );
  }
  if (!privateKey) {
    throw new Error(
      "No private key found. Ensure STACKS_PRIVATE_KEY env is set."
    );
  }
...
async function fetchPoolData(poolId: string): Promise<PoolData | null> {
  try {
    const res = await rateLimitedFetch(
      `${BFF_API_URL}/app/v1/pools/${poolId}`,
      { headers: bffHeaders() }
    );
...
async function fetchPoolBins(
  poolId: string
): Promise<Array<{ bin_id: number; price: number; reserve_x: number; reserve_y: number; liquidity: number }>> {
  const res = await rateLimitedFetch(
    `${BFF_API_URL}/quotes/v1/bins/${poolId}`,
    { headers: bffHeaders() }
  );
  if (!res.ok) throw new Error(`Pool bins API error: ${res.status}`);
  const data = await res.json();
  return data.bins ?? [];
}
...
async function pollTxConfirmation(
  txId: string,
...
function calculateAccumulatedFees(bins: PositionBin[]): {
  totalFeesX: number;
  totalFeesY: number;
} {
  return bins.reduce(
...
async function executeHarvest(
  ...
  // Only withdraw bins with accumulated fees
  const binsToHarvest = userBins.filter(
    (b) =>
      b.user_liquidity > 0 &&
      ((b.accumulated_fee_x ?? 0) > 0 || (b.accumulated_fee_y ?? 0) > 0)
  );

  if (binsToHarvest.length === 0) {
    throw new Error("No bins with accumulated fees to harvest");
  }
  ...
  const binWithdrawalPositions = binsToHarvest.map((bin) => {
    // Withdraw only fee portion — estimated as fee / total reserves ratio
    ...
    const feeRatio = Math.max(feeRatioX, feeRatioY) * 100; // percentage
    ...
    return tupleCV({
      ...
      "bin-id": intCV(getSignedBinId(bin.bin_id)),
  ...
  if (dryRun) {
    return { txId: null, harvestedX, harvestedY };
  }
  ...
  if ("error" in response) {
    throw new Error(`Harvest broadcast failed: ${JSON.stringify(response)}`);
  }
...
async function executeReinvest(
  ...
  // Reinvest into active bin using Simple (relative) mode
  const reinvestedBins = [
    {
      binId: poolData.active_bin_id,
      xAmount: harvestedX,
      yAmount: harvestedY,
    },
  ];
  ...
  if (dryRun) {
    return { txId: null, reinvestedBins };
  }

  // Use add-relative-liquidity-same-multi (Simple mode) for resilience
  const binAddPositions = [
    tupleCV({
      "active-bin-id-offset": intCV(0), // active bin
      "x-amount": uintCV(harvestedX),
      "y-amount": uintCV(harvestedY),
      "min-dlp": uintCV(0), // conservative — accept any dlp
      "max-x-liquidity-fee": uintCV(Math.ceil(harvestedX * 0.02)), // 2% max fee
  ...
  if ("error" in response) {
    throw new Error(`Reinvest broadcast failed: ${JSON.stringify(response)}`);
  }
...
async function runDoctor(options: { pool?: string }): Promise<void> {
  ...
  if (options.pool) {
    ...
    checks.pool = pool
      ? `found (activeBin: ${pool.active_bin_id})`
      : "not-found";
    if (!pool) {
      out({
        status: "error",
        checks,
        error: `Pool ${options.pool} not found`,
        timestamp: ts(),
      });
      process.exit(1);
    }
...
async function runStatus(options: { pool: string }): Promise<void> {
  ...
  const status: CompoundStatus = {
    ...
    estimatedCompoundBoostBps:
      totalLiquidity > 0 ? Math.round((feeValue / totalLiquidity) * 10_000) : 0,
  };
...
async function runCompound(options: {
  ...
  if (!options.dryRun && !options.feeCap) {
    out({
      error: "--fee-cap <STX> is required for live execution. Use --dry-run first.",
      code: "FEE_CAP_REQUIRED",
      timestamp: ts(),
    });
    process.exit(1);
  }
  ...
  const pollInterval = options.interval
    ? Math.max(600_000, parseInt(options.interval) * 1000)
  ...
  process.on("SIGINT", () => {
    out({
      ...
      compoundsExecuted: compoundCount,
      timestamp: ts(),
    });
    process.exit(0);
  });
  ...
  while (true) {
    ...
    try {
      ...
      // Execute harvest
      const { txId: harvestTxId, harvestedX, harvestedY } = await executeHarvest(
        pool,
        userBins,
        wallet.address,
        wallet.privateKey,
      ...
      if (!dryRun && harvestTxId) {
        ...
        if (!confirmed) {
          out({
            error: "Harvest transaction did not confirm within timeout — reinvest cancelled",
            code: "HARVEST_TIMEOUT",
            txId: harvestTxId,
            timestamp: ts(),
          });
          await sleep(pollInterval);
...
program
  ...
  .description(
    "Autonomously harvest and reinvest Bitflow HODLMM trading fees back into the pool"
  )
  .version("1.0.0");

program
  .command("doctor")
  .description("Validate API connectivity, wallet, and pool access")
...
program
  .command("status")
  .description("Get current position fees and compound readiness")
  .requiredOption("--pool <id>", "HODLMM pool ID to check")
  .action(async (options) => {
...
program
  .command("run")
  .description(
    "Start autonomous compound loop — harvest fees and reinvest on schedule"
  )
