...
# Agent Behavior ΓÇö HODLMM Range Rebalancer
...
5. On `"status": "rebalanced"` ΓÇö confirm tx hash and log rebalance count.
6. On `"status": "in-range"` ΓÇö wait for next poll cycle, no action.
7. On `"status": "cooldown"` ΓÇö wait out remaining cooldown, no action.
8. On `"error"` ΓÇö log payload, surface to operator, do not retry silently.
...
- Never exceed the session rebalance cap of 10 ΓÇö halt and report.
...
10. `--dry-run` is active (simulation only ΓÇö no broadcast)
...
**Out of range ΓåÆ rebalance executes:**
```json
{
  "status": "rebalanced",
