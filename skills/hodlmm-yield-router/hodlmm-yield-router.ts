 ...
 *
 * Commands:
 *   doctor  — verify all APIs and wallet are reachable
 *   status  — show HODLMM APY, Zest APY, position, recommendation
 *   run     — autonomous loop executing the routing logic
 */
...
async function decide(
  ...
  if (state.mode === "zest") {
    ...
    return {
      action: "stay",
      reason: `Staying in Zest — HODLMM APY (${hodlmmApyPct.toFixed(2)}%) has not recovered above Zest (${zestApyPct.toFixed(2)}%)`,
    };
  }
  ...
  if (!inRange && drift > BIN_DRIFT_TOLERANCE) {
    return {
      action: "rebalance",
      reason: `Active bin ${activeBinId} drifted ${drift} bins from position center — rebalancing before yield comparison`,
  ...
  if (apyGap >= ZEST_ADVANTAGE_THRESHOLD) {
    return {
      action: "move_to_zest",
      reason: `Zest APY (${zestApyPct.toFixed(2)}%) exceeds HODLMM (${hodlmmApyPct.toFixed(2)}%) by ${apyGap.toFixed(2)}% — threshold is ${ZEST_ADVANTAGE_THRESHOLD}%`,
      hodlmmApy: hodlmmApyPct,
      zestApy: zestApyPct,
    };
  }
...
program.command("run").description("Autonomous routing loop").action(async () => {
  ...
  log(`Starting yield router — mode: ${state.mode}, cycle: ${state.cycleCount}`);
